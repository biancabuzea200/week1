...
template sudoku() {
    ...
    component rangeProofToSolve[9][9];
    component rangeProofSolved[9][9];
    
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rangeProofToSolve[i][j] = RangeProof(32);
            rangeProofToSolve[i][j].in <== puzzle[i][j];
            rangeProofToSolve[i][j].range[0] <== 0;
            rangeProofToSolve[i][j].range[1] <== 9;
            rangeProofToSolve[i][j].out === 1;

            rangeProofSolved[i][j] = RangeProof(32);
            rangeProofToSolve[i][j].in <== solution[i][j];
            rangeProofSolved[i][j].range[0] <== 0;
            rangeProofSolved[i][j].range[1] <== 9;
            rangeProofToSolve[i][j].out === 1;
